...
| Path | Responsibility |
|------|----------------|
| `cmd/shuttle/` | Cobra CLI: `main.go` (root), `orchestrator.go`, `agent.go`, `enroll.go`, `prune.go`, `check.go`, `doctor.go` (`shuttle doctor`: local host preflight — config parse, git binary + repo reachability, docker daemon, data-dir writability, gRPC TLS cert parse/expiry, secrets-provider construction; pure `buildDoctorReport`/`inspectCertFile` with injectable probes, unit-tested), `version.go`, `webhook.go`, `events.go` (SSE event stream client), `init.go` (`shuttle init`: scaffold the IaC git repo — hosts/services/orchestrator.yaml/CI) + `orchestrator_init.go` (`shuttle orchestrator init`: generate the server bootstrap — config.yml/.env/TLS, repo_url auto-detected from the scaffolded repo) + `initutil.go` (shared wizard prompter with essentials-vs-`--advanced` gating + small file helpers) + `certgen.go` (self-signed orchestrator TLS cert generation for the token-enrollment path), `join.go` (`shuttle agent join`: redeem a join token over a pinned HTTPS client, persist creds, start the agent), `backup.go` (`shuttle backup`/`restore`: snapshot + restore the ledger from local files), `audit.go` (`shuttle audit`: read the control-plane audit log from a running orchestrator), `token.go` (`shuttle token create`/`list`/`revoke`: manage named, role-scoped control-plane tokens) + `backup_service.go` (`shuttle backup-service`/`backups`/`restore-service`: trigger, list, and restore *service-data* backups via a running orchestrator — distinct from `backup.go`'s ledger snapshot) + `lsp.go` (`shuttle lsp`: run the IaC language server over stdio) + `scaffold.go` (`shuttle scaffold service`/`host`/`dns-provider`/`certificate`: generate IaC files from the loader's schema — validated before write; `hosts.yaml`/`dns.yml` edits go through a yaml.Node round-trip preserving comments). Wiring only — no business logic (scaffold's render/merge is pure and unit-tested in-package). |
...
| `internal/agent/` | Agent run loop (`client.go`) + the Compose `Driver` (`compose.go`) + zero-downtime rolling strategy (`rolling.go`) + Caddy sidecar manager (`caddy.go`) + CoreDNS private-DNS sidecar (`coredns.go`: lazily-started CoreDNS container serving pushed zone files, config delivered via `docker cp` of a tar) + the backup/restore engine (`backup.go`: `Driver.Backup`/`Restore` — tar named volumes or `pg_dump`, stored to a local dir or a restic repo; backend creds via `-e KEY` passthrough, never argv). |
...
| `dns.go` | `resolveTLSPolicies` — turns the repo's `dns.yml` certificates into Caddy `tls.automation` policies (ACME DNS-01 issuer per provider), resolving provider credentials from the secrets provider and injecting them inline. Filtered per host (cert included only where the host serves a covered domain or a service pins it). `resolveDNSCreds` (shared with the record reconciler) fetches a provider's credential values from the secrets provider. `resolveInternalTLSPolicies` emits Caddy `internal`-issuer policies for `dns.yml` zones marked `internal_tls` (private-service HTTPS via Caddy's self-signed CA). |
| `dns_sidecar.go` | The `sidecar`-provider path of the DNS reconciler: `renderZoneFile` (pure RFC1035 zone-file render) + `dispatchSidecar` (push each host's full zone set as a `DNSConfigRequest` over the agent stream). Declarative full-zone push (like the Caddy config), bypassing the Manager/ledger/owner-TXT used for API providers. |
| `dns_records.go` | `DNSReconciler` — manages A/AAAA records for `dns.yml` `zones` (record management, distinct from the ACME cert path). Branches per provider: `sidecar` zones go to `dns_sidecar.go`; `ovh`/`manual` go through the `internal/dns` `Manager`. Ticks like the drift reconciler (reads the synced working copy); `buildDesiredRecords` = each non-external service's domains × matched zone × host `Address(zone.address)`; diffs desired ⟷ provider-owned (`internal/dns` `Manager.Owned`) ⟷ the `dns_records` ledger → create / heal out-of-band drift / delete-if-owned. `manual`-provider zones are surfaced not applied. Publishes `dns.record_set`/`dns.record_removed`/`dns.drift_healed`. `CheckDNSRecords` (in `check.go`) reuses `buildDesiredRecords` to surface managed records + host-address problems in `shuttle check`. |
...
| `events.go` | `EventBus` — in-process pub/sub for orchestrator events (`deploy.queued/log/succeeded/failed/rolled_back`, `rollback.queued`, `drift.detected`, `service.removed`, `volumes.purged`, `dns.record_set/record_removed/drift_healed`). Publishers: `dispatch`, the deploy-result + deploy-log handlers, the drift reconciler, teardown/purge. Bounded per-subscriber buffers (drop on overflow) + a replay ring. Consumed by the SSE stream (`/events`), metrics (`metrics.go`), and outbound notifications (`notify.go`). |
...
- **Bootstrap split into two commands along the two-tier config seam: `shuttle init` (IaC repo) + `shuttle orchestrator init` (server config).** A single all-in-one wizard conflated two concerns with different owners — the git-managed IaC repo vs the server-only bootstrap — and asked ~20 questions. They're now two focused, secure-by-default commands, each **short** (essentials only) with everything else behind `--advanced`; both take `--dir` (default `.`) and scaffold into one shared project directory (no nested sub-folder, so an editor opening the root finds `hosts.yaml`/`services/` at top level). The split mirrors the `config.yml` (bootstrap) vs `orchestrator.yaml`/IaC (git-managed) config split exactly.
  - **`shuttle init`** scaffolds only the git-managed repo: `hosts.yaml`, `services/`, `orchestrator.yaml`, `.gitignore` (keeps `config.yml`/`.env`/`certs/`/`data/`/`*.db` out of git), provider-appropriate CI files, + an initial commit. Two essential questions — **starter** (`whoami` example + a `local` host) vs **empty** scaffold, and the **CI provider** you'll push to: `none`/`github` (`.github/workflows/` deploy + plan-comment)/`gitlab` (`.gitlab-ci.yml` deploy + MR-plan); `--ci github|gitlab|none` pre-answers it (`normalizeCIProvider`, `scaffoldCI`). `--advanced` adds `orchestrator.yaml` overrides (Caddy, secret paths). It is **remote-agnostic on purpose** — the remote may not exist at scaffold time, so `init` never touches a git remote; the user creates the remote, pushes, then runs `orchestrator init --repo-url <url>` on the server (the cross-machine flow). The old "existing remote" mode is gone — if you already have a repo you skip `init` entirely. `promptRepoInit`/`applyRepoInit` are split for stdin-free unit tests.
  - **`shuttle orchestrator init`** generates the server bootstrap: `config.yml` (+ `.env` for Infisical creds, mode 0600; incl. `advertise_control_url` so `shuttle enroll --config` needs no `--url`) and, on the default token path, **generates the orchestrator's self-signed EC cert inline** (`certgen.go`, `ensureSelfSignedCert`; SANs = advertise server name + `localhost`/`127.0.0.1` + advertised gRPC/control hosts via `certSANs`, never clobbered). Two essential questions — agent transport (token-TLS default / mTLS / insecure) and secrets provider (none/infisical/file); `--advanced` exposes addresses, advertise URL/SAN, cert paths, secret paths, and bearer/webhook overrides (auto-generated 32-byte `crypto/rand` hex otherwise). It **auto-detects the scaffolded repo** in `--dir` to fill `repo_url` (`detectRepoURL`): an existing `origin` remote is reused; a local repo with no remote is driven directly via `file://<abs>` (a real, secured first deploy with nothing to push); `--repo-url` overrides. `promptOrchInit`/`applyOrchInit` split for testability.
  - Shared wizard plumbing lives in `initutil.go`: a `prompter` whose `adv`/`advBool` helpers silently return defaults unless `--advanced` (the mechanism that keeps the default runs short), plus `generateHexToken`/`writeFileIfAbsent`/`resolveUnderOutput`/`joinRepo`. Both commands are idempotent (never overwrite user-edited files, incl. a real cert).
...
- **DNS-challenge certificates: `dns.yml` in the repo, secrets from the
  ...
  **decoupled**: `dns.yml` owns the cert lifecycle; a service just declares its
  ...
  credentials resolve.
- **DNS record management: `dns.yml` `zones` map a domain → provider, ledger +
  owner-TXT ownership, drift-healed.** The cert flow above proves domain ownership
  (ACME DNS-01 TXT) but never creates the **A/AAAA** record pointing a name at a
  host — that was manual. `zones:` closes the gap (an external-dns-style
  reconciler): each entry binds a domain suffix to a **record-management**
  provider and a host **address label**, and the `DNSReconciler` upserts
  `<service-domain> A/AAAA → host.Address(label)` for every covered domain. This
  is a **separate capability axis** from ACME — `dnsProviderSpec` carries `acme`
  vs `records` flags, so `certificates[]` need an ACME-capable provider and
  `zones[]` a record-capable one. The design serves **split DNS** (the motivating
  case): a public zone via `ovh` + a private (Tailscale) zone via the sidecar,
  selected per-domain by longest-suffix match, each pointing at a different host
  address label (`public`/`tailscale`). **Supported record providers: `ovh`
  (libdns/ovh) + `manual` (no-op — Shuttle creates nothing, `check`/`plan` list
  what you must create) + `sidecar` (a CoreDNS sidecar on a named host serves a
  private zone — see the sidecar paragraph below); `cloudflare`/`route53` stay
  ACME-only until their record managers land.**
  **Ownership is conservative and double-booked** (API providers like `ovh`)**:** every managed record gets a
  companion **owner TXT** (`_shuttle-owner.<name>` = `heritage=shuttle`) *and* a
  `dns_records` **ledger** row, so the reconciler diffs desired ⟷ provider-owned
  ⟷ ledger to **create**, **heal** out-of-band value drift, and **delete** records
  whose service/domain left the repo — while **never touching foreign records**
  (no owner TXT = not ours). Ledger gives exact create/delete knowledge + offline
  `check`/`plan`; the TXT survives ledger loss and detects drift on the provider
  side (both, per the explicit decision to cross-check). Removing a `zone` stops
  management without deleting its records (no surprise teardown). Provider
  credentials are secrets resolved per reconcile via the shared
  `resolveDNSCreds` (never on disk), and the reconciler runs on its own slower
  tick (2m) to be gentle on provider APIs. Record CRUD goes through the
  `internal/dns` `Manager` interface (`libdns` for real providers — the same
  ecosystem the Caddy image already uses), keeping the orchestrator provider-agnostic.
  **The `sidecar` provider is the private-DNS path and bypasses the Manager/ledger/owner-TXT machinery** — it's declarative like the Caddy push, not API record CRUD. A sidecar provider names a `host` (+ optional `port`, default 53). On each tick the reconciler renders an RFC1035 zone file per sidecar zone (`dns_sidecar.go` `renderZoneFile` — SOA/NS authority with the host's own address as glue, sorted records, serial bump) and pushes the full zone set for a host as one `DNSConfigRequest` (proto) via the agent stream. The **agent** (`internal/agent/coredns.go`) starts a **CoreDNS** container lazily on the first push (so only the named host runs it), delivering the Corefile + zone files by `docker cp` of a tar (shell-free → works on the distroless CoreDNS image) and letting CoreDNS's `reload`/`file` plugins auto-apply; the container is recreated only when absent or its published `:53` port changed (label-stamped, like the Caddy sidecar). The zone file is the source of truth (no owner TXT/ledger — the whole zone is Shuttle's). Point a private resolver at it (Tailscale split-DNS → the sidecar host's tailnet IP). Image default `coredns/coredns`, `--dns-image` override on `agent`/`agent join`. **Record kind is inferred from the host address**: an IP → `A`/`AAAA`, a hostname → `CNAME` (e.g. a load balancer / MagicDNS name); `recordType` classifies, and both the ovh Manager (`libdns.CNAME`) and the sidecar zone-file render emit it. **Private-service HTTPS** is opt-in per zone via `internal_tls: true` → the orchestrator emits a Caddy `tls.automation` policy with the **`internal`** (self-signed CA) issuer for that zone's served domains (`resolveInternalTLSPolicies`, merged into both the per-host sidecar-Caddy push and the central `ApplyRoutes`), since a private zone can't pass public ACME (clients must trust Caddy's local CA). The reconcile cadence is tunable via `dns_reconcile_interval` (config.yml, default 2m). E2E: `test/integration` `TestDNSSidecarResolves` (orchestrator+agent → CoreDNS resolves a service domain to the host address; self-skips without Docker).
