...
- **DNS record management: `dns.yml` `zones` map a domain → provider, ledger +
  ...
  what you must create); `sidecar` (CoreDNS private DNS) is the planned next
  slice; `cloudflare`/`route53` stay ACME-only until their record managers land.**
  **Ownership is conservative and double-booked:** every managed record gets a
