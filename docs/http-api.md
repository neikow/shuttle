...
Event types: `deploy.queued`, `deploy.log`, `deploy.succeeded`, `deploy.failed`,
`deploy.rolled_back`, `rollback.queued`, `drift.detected`, `service.removed`,
`volumes.purged`.

`deploy.log` carries a chunk of live output from an in-progress
deploy/rollback/teardown (`deploy_id`, `service`, and a newline-joined `message`),
streamed best-effort so a client can tail a deploy as it runs. It is **not**
persisted — the complete, point-in-time logs are written when the deploy finishes
and read from [`GET /deploys/{id}/logs`](#get-deploysidlogs). The UI's per-deploy
**Logs** view tails these events for a running deploy and falls back to the
persisted logs once it is terminal.
